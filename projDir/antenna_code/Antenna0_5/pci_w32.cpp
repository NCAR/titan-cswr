...
PCI_CARD *find_pci_card(int vendorid,int deviceid,int n)
{
	...
	buses = GetLastPciBus(PCI_HANDLE);
...
BYTE *pci_card_membase(PCI_CARD *card,int which,int length)
{
	unsigned long phys = (which == 1) ? card->phys : card->phys2;
	card->base = (BYTE *)MapPhysToLinear(PCI_HANDLE,phys,length);
