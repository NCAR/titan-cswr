...
def main():
    ...
    beginString = "BEGIN: putImagesToCswr.py"
    ...
    fieldName = ""
    if (options.fileName.find("DBZ") > 0):
        fieldName = "DBZ"
    elif (options.fileName.find("VEL") > 0):
        fieldName = "VEL"
    elif (options.fileName.find("ZDR") > 0):
        fieldName = "ZDR"
    elif (options.fileName.find("RHOHV") > 0):
        fieldName = "RHOHV"
    else:
    ...
    ftpFile(options.fileName, options.fullFilePath, validDayStr, fieldName)
    ...
    if (options.debug == True):
        print >>sys.stderr, "==================================================="
        print >>sys.stderr, \
              "END: putImagesToCswr.py at " + str(datetime.datetime.now())
        print >>sys.stderr, "==================================================="
...
def ftpFile(fileName, filePath, validDayStr, fieldName):
    ...
    targetDir = "public/OWLeS/images/" + options.radarName + "/" + options.frequency
    targetDir += "/"
    targetDir += fieldName
    targetDir += "/"
